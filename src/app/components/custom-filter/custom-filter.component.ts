...
export class CustomFilterComponent {
    ...
    public dateCtrl = new FormControl<MonthYearToggleType | null>(null);
    ...
    ngOnInit() {
        ...
        const dateFilter = this.verifyDateRanges(hasMonth, hasYear);
        if (dateFilter) this.dateCtrl.patchValue(dateFilter as MonthYearToggleType);
    ...
    verifyDateRanges(month?: FilterDisplay, year?: FilterDisplay) {
        ...
        const prevDate = getMonthAndYearIntegers(-1);
        const currentDate = getMonthAndYearIntegers(0);
        const nextDate = getMonthAndYearIntegers(1);

        if (
            isObjectsEqual(dateObject, prevDate) ||
            isObjectsEqual(dateObject, nextDate) ||
            isObjectsEqual(dateObject, currentDate)
        )
            return dateObject;
    ...
    setFiltersAndFetchBills(filter: FilterDisplay) {
        this.filterState.addFilters([filter]);
        this.getBills();
    }
    ...
    addDateSpan(event: ToggleButtonItemsType<MonthYearToggleType>) {
